...
const canvasToTIFF = (canvas: HTMLCanvasElement): Promise<Blob> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const tiffData = UTIF.encodeImageData(imageData);
...
const PDFToMultiFormatConverter: React.FC = () => {
  ...
  const [settings, setSettings] = useState({
    ...
    splitThreshold: 1.7,
    outputFormat: 'jpg' as 'jpg' | 'png' | 'webp' | 'tiff'
  });
...
const PDFToMultiFormatConverterWithProvider: React.FC = () => (
  <NotificationProvider>
    <PDFToMultiFormatConverter />
  </NotificationProvider>
);

export default PDFToMultiFormatConverterWithProvider; 